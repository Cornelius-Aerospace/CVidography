/*
  ...
  CVideography - CFlight Flight Recorder Module

  This program records an mjpeg avi video to the sd card of an ESP32-CAM.
  It communicates with the CFlight-FC (Flight Controller) which tells it when to begin and
  end recording; Recording commences 5 secconds prior to fire, if no launch is detected
  within 30 secconds the reccording ends. Otherwise the recording continues until landing is
  detected by the FC
  ...
  The files will have the name such as:
    ...
    "cflight" is your device name, you can change it in software
    10 - is a number stored in eprom that will increase everytime your CFlight system is enabled
    3 - is the 3rd file created during the current boot. In a single, succsesfull flight you
        should have 1 file on here. As each flight will require a reboot of the system for rocket
        set-up (bp-charge loading should NEVER be done with the fc enabled) you can think of the "10"
        here as the flight number.

  Small red led on the back blinks with every frame.
  Next to the camera is an LED. This led will blink if there is an issue preflight (no/full SD, failed
  to handshake with FC etc)

  by Leah Cornelius (of Cornelius Aerospace)
     corneliusaerospace@gmail.com

  based on code by James Zahary Sep 12, 2020
...
// Example time zones
// String TIMEZONE = "GMT0BST,M3.5.0/01,M10.5.0/02";     // UK
// String TIMEZONE = "MET-2METDST,M3.5.0/01,M10.5.0/02"; // Most of Europe
// String TIMEZONE = "CET-1CEST,M3.5.0,M10.5.0/3";       // Central Europe
// String TIMEZONE = "EST-2METDST,M3.5.0/01,M10.5.0/02"; // Most of Europe
// String TIMEZONE = "EST5EDT,M3.2.0,M11.1.0";           // EST USA
// String TIMEZONE = "CST6CDT,M3.2.0,M11.1.0";           // CST USA
// String TIMEZONE = "MST7MDT,M4.1.0,M10.5.0";           // MST USA
// String TIMEZONE = "NZST-12NZDT,M9.5.0,M4.1.0/3";      // Auckland
// String TIMEZONE = "EET-2EEST,M3.5.5/0,M10.5.5/0";     // Asia
// String TIMEZONE = "ACST-9:30ACDT,M10.1.0,M4.1.0/3":   // Australia

String TIMEZONE = "GMT0BST,M3.5.0/01,M10.5.0/02";
#define Lots_of_Stats 1 // Controlls UART logging level, disable for production use [TODO]
#define blinking 0
...
int MagicNumber = 7;     // change this number to reset the eprom in your esp32 for file numbers
...
int begin_flight = 0;
int end_flight = 0;
int ready = 0; // Sent to FC after all setup has been complete
...
//
// if we have no camera, or sd card, then flash front led on and off to warn the human SOS - SOS
//
void major_fail()
{
...
bool init_wifi()
{
  ...
  if (IncludeInternet == 3 || IncludeInternet == 4)
  {
    ...
    while (WiFi.status() != WL_CONNECTED)
    {
      ...
      Serial.print(".");
      if (connAttempts++ == 15)
        break; // try for 15 seconds to get internet, then give up
    }
...
void setup()
{
  ...
  pinMode(12, INPUT_PULLUP); // pulled down to stop recording by FC
  pinMode(13, INPUT_PULLUP); // pulled down to start recording by FC

  // Serial.setDebugOutput(true);

  Serial.println("                                    ");
  Serial.println("-------------------------------------");
  Serial.printf("CFlight Videography Module %s\n", vernum);
  Serial.println("-------------------------------------");
  ...
  */
  ...
  // [TODO] Let FC know we are ready to record
}
...
void the_camera_loop(void *pvParameter)
{
  ...
  while (1)
  {
    ...
    else if (frame_cnt == 0 && start_record == 1)
    {
      ...
      Serial.println("Starting recording CVideography A-go!");
...
void loop()
{
  ...
  {
    // We are recording, check for stop recording signal
    int stop_recording = digitalRead(13);
    delay(20);
    stop_recording = stop_recording + digitalRead(13); // get 2 opinions to help poor soldering
    if (stop_recording < 2) {
      start_record = 0;
      Serial.println("Stopped recording signal recevied!");
    }
  }
